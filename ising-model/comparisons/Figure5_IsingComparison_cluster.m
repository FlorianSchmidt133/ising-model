...
% =========================================================================
% CLUSTER MODE: Headless figure generation (no display required)
% =========================================================================
set(0, 'DefaultFigureVisible', 'off');
fprintf('Running in CLUSTER MODE: Figures will be saved to files (no display)\n\n');
...
config.autocorr.fitRange = [1, 50];     % Lag range for exponential decay fit
...
% Use local scratch if available (set by SLURM script), otherwise NFS
isingPathEnv = getenv('ISING_DATA_PATH');
if ~isempty(isingPathEnv)
    config.isingDataPath = isingPathEnv;
    fprintf('Using local scratch: %s\n', config.isingDataPath);
else
    config.isingDataPath = '/path/to/data/IsingSims';
    fprintf('WARNING: Using NFS path (slower)\n');
end
...
% Data Paths
% -------------------------------------------------------------------------
% Experimental data path - use environment variable if available
expPathEnv = getenv('EXP_DATA_PATH');
if ~isempty(expPathEnv)
    config.experimentalDataPath = expPathEnv;
else
    config.experimentalDataPath = '/path/to/data/ExperimentalData/ExperimentalData.mat';
end
...
% Start parallel pool
nWorkers = min(24, feature('numcores'));
pool = gcp('nocreate');
if isempty(pool)
...
parfor i = 1:nSimFiles
    ...
    moransI_timeseries = zeros(1, nFrames, 'single');  % Use single precision to save memory

    % For subselect_tiled: also store per-position time series
    if strcmp(config.gridMode, 'subselect_tiled')
        moransI_perPos = zeros(nTiledPositions, nFrames, 'single');  % Use single precision
    end

    % For subselect_centre_vs_tiled: store centre and per-position tiled
    if strcmp(config.gridMode, 'subselect_centre_vs_tiled')
        moransI_centre_ts = zeros(1, nFrames, 'single');  % Use single precision
        moransI_tiled_perPos = zeros(nTiledPositions, nFrames, 'single');  % Use single precision
    end
    ...
    temp_Activity_all{i} = single(activity_timeseries(:)');  % Store as row vector (single precision)
...
% -------------------------------------------------------------------------
% Figure 4: Autocorrelation Comparison
% -------------------------------------------------------------------------
...
for c = 1:nConditions
    ...
    if ~isempty(DynamicsAnalysis.(condition).exp_acf)
        plot(DynamicsAnalysis.(condition).exp_lags, DynamicsAnalysis.(condition).exp_acf, ...
            'b-', 'LineWidth', 2, 'DisplayName', 'Experimental');
    end
    ...
    plot(DynamicsAnalysis.(condition).ising_lags, DynamicsAnalysis.(condition).ising_acf, ...
        ...
        sprintf('Ising %s', DynamicsAnalysis.(condition).best_simID));

    % Add fitted exponential decay curves if time constants are available
    if isfield(DynamicsAnalysis.(condition), 'exp_tau') && isfield(DynamicsAnalysis.(condition), 'ising_tau')
        ...
        if ~isnan(exp_tau) && ~isnan(ising_tau)
            ...
            plot(lags_fit, exp_fit, 'b--', 'LineWidth', 1.5, ...
                'DisplayName', sprintf('Exp fit (τ=%.1f)', exp_tau));
            plot(lags_fit, ising_fit, 'r--', 'LineWidth', 1.5, ...
                'DisplayName', sprintf('Ising fit (τ=%.1f)', ising_tau));
            ...
            if isfield(DynamicsAnalysis.(condition), 'time_scaling') && ...
               ~isnan(DynamicsAnalysis.(condition).time_scaling)
                text(0.5, 0.85, sprintf('1 MC = %.2f frames', DynamicsAnalysis.(condition).time_scaling), ...
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: Ising Parameter Space (Top 10)');

    % Plot all simulations in gray
    scatter(embedding(:,1), embedding(:,2), 20, [0.7 0.7 0.7], 'filled', 'MarkerFaceAlpha', 0.3);
    hold on;

    % Highlight top 10 best matches for each condition
    for c = 1:length(config.conditions)
        condition = config.conditions{c};
        if isfield(Comparison, condition)
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('Ising Parameter Space (UMAP) with Top 10 Matches per Condition');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_ParameterSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: Moran''s I Feature Space (Top 10)');
    ...
    % Highlight top 10 best-matching Ising simulations for each condition
    for c = 1:nConds
        condition = config.conditions{c};
        if isfield(Comparison, condition)
            nTop = min(10, length(Comparison.(condition).bestMatch_idx));
            ...
            scatter(embedding_ising(best_idx, 1), embedding_ising(best_idx, 2), 100, ...
                conditionColor, 'filled', 'MarkerEdgeColor', 'k', 'LineWidth', 1.5, ...
                'DisplayName', [condition ' (Top 10 Ising)']);
        end
    end
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('Moran''s I Feature Space: Top 10 Matches per Condition');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_MoransI_FeatureSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: Activity Feature Space (Top 10)');
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('Activity Feature Space: Top 10 Matches per Condition');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_Activity_FeatureSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: Moran''s I + Activity Feature Space (Top 10)');
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('Moran''s I + Activity Feature Space (Top 10 Matches)');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_MoransI_Activity_FeatureSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: Combined Feature Space (Top 10)');
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('Combined Feature Space: Top 10 Matches per Condition');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_Combined_FeatureSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: Spatial+Persistence Feature Space (Top 10)');
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('Spatial+Persistence Feature Space: Top 10 Matches per Condition');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_SpatialPersistence_FeatureSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
...
try
    ...
    % --- Top 10 version of same UMAP ---
    figure('Name', 'UMAP: BlobPersistence Feature Space (Top 10)');
    ...
    xlabel('UMAP 1');
    ylabel('UMAP 2');
    title('BlobPersistence Feature Space: Top 10 Matches per Condition');
    legend('Location', 'best');

    % Save figure
    saveMyFig('UMAP_BlobPersistence_FeatureSpace_Top10', fullfile(config.outputPath, 'UMAP'), gcf);
