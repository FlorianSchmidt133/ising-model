...
%                     Default: 1
%
%   'violinmin'       Truncation bounds for violins at the lower end.
%                     Can be:
%                       - Scalar: Same lower bound for all violins
%                       - Vector: One value per group (length = num_groups)
%                       - Cell array: One vector per group, each with values
%                                     per location/bin (length{g} = num_locs)
%                     Default: no lower limit
%
%   'violinmax'       Truncation bounds for violins at the upper end.
%                     Can be:
%                       - Scalar: Same upper bound for all violins
%                       - Vector: One value per group (length = num_groups)
%                       - Cell array: One vector per group, each with values
%                                     per location/bin (length{g} = num_locs)
...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        if ~isempty(confs.violinmin) || ~isempty(confs.violinmax)
            ...
            % Handle scalar, vector, or cell array inputs
            if ~isempty(confs.violinmin)
                if iscell(confs.violinmin)
                    % Cell array: violinmin{g}(k) - per group-location bounds
                    if length(confs.violinmin) >= g && length(confs.violinmin{g}) >= k
                        xmin = confs.violinmin{g}(k);
                    else
                        xmin = min(xi);  % Fallback if index out of range
                    end
                elseif length(confs.violinmin) > 1
                    % Vector: violinmin(g) - per group bounds
                    if length(confs.violinmin) >= g
                        xmin = confs.violinmin(g);
                    else
                        xmin = min(xi);  % Fallback if index out of range
                    end
                else
                    % Scalar: same bound for all violins
                    xmin = confs.violinmin;
                end

                % Skip truncation if bound is NaN (per-violin opt-out)
                if isnan(xmin)
                    xmin = min(xi);
                end
            else
            ...
            if ~isempty(confs.violinmax)
                if iscell(confs.violinmax)
                    % Cell array: violinmax{g}(k) - per group-location bounds
                    if length(confs.violinmax) >= g && length(confs.violinmax{g}) >= k
                        xmax = confs.violinmax{g}(k);
                    else
                        xmax = max(xi);  % Fallback if index out of range
                    end
                elseif length(confs.violinmax) > 1
                    % Vector: violinmax(g) - per group bounds
                    if length(confs.violinmax) >= g
                        xmax = confs.violinmax(g);
                    else
                        xmax = max(xi);  % Fallback if index out of range
                    end
                else
